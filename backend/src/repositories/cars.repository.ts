...
export class CarRepository {
  async create(data: CreateCarData): Promise<Car> {
    ...
    try {
      const result = await pool.query(
        ...
        RETURNING *`,
        [
          data.make, data.model, data.variant ?? null, data.year, data.price,
          data.mileage, data.condition, data.body_type ?? null, data.transmission,
          data.fuel_type, data.color ?? null, data.description ?? null,
          featuresJson, data.status, data.video_url ?? null,
        ]
      );
  ...
  async findAll(filters: CarFilters): Promise<{ cars: Car[]; total: number }> {
    ...
    const sortColumn = ALLOWED_SORT_COLUMNS.includes(filters.sortBy) ? filters.sortBy : 'created_at';
    const sortOrder = filters.sortOrder === 'asc' ? 'ASC' : 'DESC';

    const countResult = await pool.query(`SELECT COUNT(*) FROM cars c WHERE ${whereClause}`, params);
  ...
  async findById(id: string): Promise<Car | null> {
    ...
    const imagesResult = await pool.query(
      `SELECT id, car_id, s3_key, cloudfront_url, thumbnail_url, is_primary, order_index, created_at
       FROM car_images WHERE car_id = $1 ORDER BY is_primary DESC, order_index ASC`,
      [id]
    );
  ...
  async update(id: string, data: Partial<CreateCarData>): Promise<Car | null> {
    ...
    if (data.make !== undefined) { updates.push(`make = $${paramIndex++}`); params.push(data.make); }
    if (data.model !== undefined) { updates.push(`model = $${paramIndex++}`); params.push(data.model); }
    if (data.variant !== undefined) { updates.push(`variant = $${paramIndex++}`); params.push(data.variant || null); }
    if (data.year !== undefined) { updates.push(`year = $${paramIndex++}`); params.push(data.year); }
    if (data.price !== undefined) { updates.push(`price = $${paramIndex++}`); params.push(data.price); }
    if (data.mileage !== undefined) { updates.push(`mileage = $${paramIndex++}`); params.push(data.mileage); }
    if (data.condition !== undefined) { updates.push(`condition = $${paramIndex++}`); params.push(data.condition); }
    if (data.body_type !== undefined) { updates.push(`body_type = $${paramIndex++}`); params.push(data.body_type || null); }
    if (data.transmission !== undefined) { updates.push(`transmission = $${paramIndex++}`); params.push(data.transmission); }
    if (data.fuel_type !== undefined) { updates.push(`fuel_type = $${paramIndex++}`); params.push(data.fuel_type); }
    if (data.color !== undefined) { updates.push(`color = $${paramIndex++}`); params.push(data.color || null); }
    if (data.description !== undefined) { updates.push(`description = $${paramIndex++}`); params.push(data.description || null); }
    if (data.features !== undefined) { updates.push(`features = $${paramIndex++}::jsonb`); params.push(JSON.stringify(data.features ?? [])); }
    if (data.status !== undefined) { updates.push(`status = $${paramIndex++}`); params.push(data.status); }
    if (data.video_url !== undefined) { updates.push(`video_url = $${paramIndex++}`); params.push(data.video_url || null); }
    ...
    return result.rows.length === 0 ? null : result.rows[0] as Car;
  }
  ...
  async fullTextSearch(query: string | null, filters: SearchFilters, options: SearchOptions): Promise<SearchResult> {
    const pool = await getPool();
    const params: unknown[] = [];
    let paramIndex = 1;
    ...
    if (options.sortBy && options.sortBy !== 'relevance') {
      ...
      orderBy = `c.${options.sortBy} ${sortDirection}`;
    } else if (query) {
    ...
    return { hits: dataResult.rows, total };
  }
